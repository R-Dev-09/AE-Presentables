...
export class PresentablesComponent implements OnInit {
  ...
  setIsNew(presId: number) {
    if (this.isNew) {
      ...
      this.dataService.onUpdatePresentable({creator, isNew: false}, presId);
    }
  }
  ...
  onEdit(presId: number) {
    ...
    this.dataService.onUpdatePresentable(createdP, presId);
    this.editMode = false;
    this.isLoadingE = false;
    this.dataService.onGetPresentables(this.authService.getUserEmail());
  }
